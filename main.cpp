...
void pauseGame(Table table) {
	...
	ofs << coin;
	ofs << "\n";
	while (players.front().getHand().getNumCards() != 0) {
		string n = (const char*)players.front().getHand().top()->getName().at(0);
		players.front().getHand().play();
		ofs << n;
	}
	ofs << "\n";
	...
	ofs << coin;
	ofs << "\n";
	while (players.back().getHand().getNumCards() != 0) {
		string n = (const char*)players.back().getHand().top()->getName().at(0);
		players.back().getHand().play();
		ofs << n;
	}
	ofs << "\n";
			...
			for (auto& player : players) {
				...
				do {
					...
					for (int i = 0; i < player.getNumChains(); ++i) {
						...
						if (card->getName() == chain->getChainType()) {
							cout << "Card will be added to existing chain " << i+1 << "." << endl;
							// Add card
							if (card->getName() == "Quartz") {
								dynamic_cast<Chain<Quartz>*>(chain)->operator+=(card);
							}
					...
					if (!played) {
						if (player.getNumChains() < player.getMaxNumChains()) {
							cout << "Card will be added to empty chain " << player.getNumChains()+1 << "." << endl;
